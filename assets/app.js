...
const TOOL_GUIDES = {
  "황금비 측정 실험실": {
    usage: "메인 화면에서 예시 사진을 선택하거나 ‘사진 추가’로 직접 촬영한 사진을 불러옵니다. ‘측정’ 도구에서 비교할 직사각형의 첫 번째 꼭짓점과 마주 보는 두 번째 꼭짓점을 차례로 누르거나 한 번에 드래그하세요. 오른쪽 결과 카드에서 가로·세로, 긴 변 ÷ 짧은 변, 황금비와의 차이와 오차율을 확인합니다.",
    tips: "↶ 90°와 90° ↷ 버튼으로 빠르게 회전하거나 회전 슬라이더를 움직여 −180°부터 180°까지 1° 단위로 기울기를 맞출 수 있습니다. +/− 버튼, 확대 슬라이더, 마우스 휠로 확대·축소하고 ‘이동’ 도구나 스페이스바를 누른 채 드래그하면 사진 위치가 바뀝니다. 측정 뒤 A·B 점을 끌어 경계를 세밀하게 조정하고, ‘황금 사각형 비교’를 켜면 이상적인 황금 사각형이 점선으로 표시됩니다.",
    purpose: "자연물·건축물·생활용품의 가로세로 비율을 직접 측정하고 황금비 φ ≈ 1.618과 비교합니다. ‘황금비처럼 보인다’는 인상과 실제 측정값을 구분하고, 측정 위치에 따라 값이 달라지는 이유를 수치와 오차율로 설명하는 탐구 활동에 활용합니다.",
    ...
    studentSteps: "사진 선택 → 측정할 대상과 경계 정하기 → 두 꼭짓점 지정 → 측정 비율과 오차율 기록 → 점 위치를 조정해 값의 변화 관찰 → 다른 대상 또는 다른 모둠 결과와 비교 → 황금비에 가깝다고 판단한 근거 설명의 순서로 진행합니다.",
    flow: "황금비일 것 같은 대상 예상 → 예시 사진으로 측정 방법 익히기 → 직접 준비한 사진 측정 → 황금 사각형 비교선 관찰 → 측정 위치별 결과 비교 → 가장 가까운 사례와 그렇지 않은 사례 발표 → ‘가깝다’의 기준 토론",
    ...
    questions: "어떤 두 점을 경계로 정했나요? 그 경계가 달라지면 비율은 어떻게 변하나요? 오차율이 몇 % 이하면 황금비에 가깝다고 말할 수 있을까요? 자연이나 건축에 황금비가 있다는 주장을 한 번의 측정만으로 확정할 수 있을까요?",

...
export function MissionTab({ batters, answers, onAnswersChange, onApplyMission }: Props) {
  ...
  return <section className="tab-page"><div className="section-heading"><div><p className="overline dark">STEP 05 · MISSIONS</p><h2>감독처럼 판단하고, 확률로 설명하세요</h2><p>미션을 열면 관련 주자 상황과 추천 정렬이 선수 카드에 자동으로 적용됩니다.</p></div><div className="mission-progress"><strong>{completed}</strong><span>/ {MISSIONS.length} 완료</span></div></div>

...
1. 내려받은 ZIP 파일에서 마우스 오른쪽 버튼을 누르고 **모두 압축 풀기**를 선택합니다.
2. 압축을 푼 폴더의 **동영상 용량 줄이기 실행.cmd**를 더블클릭합니다.
3. 처음 실행할 때 필요한 프로그램이 없으면 실행 도우미가 자동 설치를 진행합니다. Windows 확인 창이 나타나면 허용해 주세요.
4. 설치가 끝났다는 안내가 나오면 실행 파일을 한 번 더 더블클릭합니다.
5. 인터넷 창이 자동으로 열리면 영상을 끌어다 놓고 **영상 줄이기 시작**을 누릅니다.

아래의 Node.js와 FFmpeg 수동 설치 방법은 실행 도우미로 준비되지 않을 때만 확인하면 됩니다.
